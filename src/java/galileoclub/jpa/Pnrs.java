...
@NamedQueries({
    ...
    @NamedQuery(name = "Pnrs.findByPnrsVersion",
    query = "SELECT p FROM Pnrs p WHERE p.pnrsVersion = :pnrsVersion")
})
public class Pnrs implements Serializable {
    ...
    public int hashCode() {
        int hash = 0;
        hash += (pnrsId != null ? pnrsId.hashCode() : 0);
        return hash;
    }

    @Override
    public boolean equals(Object object) {
        if (!(object instanceof Pnrs)) {
            return false;
        }
        Pnrs other = (Pnrs) object;
        if ((this.pnrsId == null && other.pnrsId != null) || (this.pnrsId != null && !this.pnrsId.equals(other.pnrsId))) {
            return false;
        }
        return true;
    }
